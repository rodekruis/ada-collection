...
from abd_model.tiles import tile_pixel_to_location, tiles_to_geojson
...
def load_config(path):
    ...
    if not path and "ABD_CONFIG" in os.environ:
        path = os.environ["ABD_CONFIG"]
    if not path and os.path.isfile(os.path.expanduser("~/.abd_config")):
        path = "~/.abd_config"
    assert path, "Either ~/.abd_config or ABD_CONFIG env var or --config parameter, is required."
